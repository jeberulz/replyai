...
The next program deliberately starts at WP41. WP34 is an unused historical gap,
and work-package identifiers are never reused or backfilled. Dependencies below
are merge gates: a worker does not start from an unmerged dependency branch.

| # | Package | Phase | Dependencies | Key files/areas | Definition of done |
|---|---|---|---|---|---|
| WP41 | Grok + Evaluation Lab program registration | P0 | WP40 strategy baseline only; no runtime dependency | `docs/PRODUCT_STRATEGY.md`, `docs/wp/WP41-GROK-EVAL-PROGRAM.md`, `docs/wp/wp41-{stories,progress}.md` | WP42–WP51 are registered once with explicit ownership, dependency/collision rules, routing limitations, rollout/owner gates, and verifiable wave gates; docs consistency checks pass |
| WP42 | Provider-neutral AI foundation | P0 | WP41 | `src/lib/env.ts`, `src/lib/ai.ts`, `shared/models.ts`, new `src/lib/providers/**` + shared provider contracts, `shared/demoData.ts`, typed provider analytics, focused tests | Server-only xAI configuration defaults to pinned `grok-4.3` at low discovery effort; authenticated `/v1/models` entitlement, normalized usage/latency/tool/reasoning/cached-token/cost/error data, redaction, provider-neutral discovery/generation contracts, and deterministic zero-key fallback are tested; no scanner or UI integration |
| WP43 | Grok X Search + authoritative X hydration | P0 | WP42 | xAI adapter under `src/lib/providers/**`, `src/lib/x.ts`, new shared citation/X-URL parsing and discovery guardrails, focused provider/security fixtures and tests | A bounded Responses API `x_search` call returns strict schema-valid cited candidates; date/handle/media/result/timeout/successful-tool-call limits and prompt-injection defenses are enforced; canonical IDs are independently hydrated through X before validity; failures are normalized and publish/schedule paths are unreachable; no user-visible opportunity writes |
| WP44 | Evaluation domain, operator authorization + account-data compatibility | P0 | WP42 contracts stable | `convex/schema.ts`, new `convex/eval*.ts` domain/auth modules, `convex/helpers.ts`, `shared/eval*.ts`, `shared/accountData.ts`, `convex/account.ts`, historical `convex/evals.ts`/`modelEvals` compatibility tests | Additive versioned datasets/cases/experiments/runs/outputs/judgments/decisions exist with required indexes; every public lab function and export is server-authorized for operators, client input uses catalog IDs rather than arbitrary model IDs, account export/deletion covers applicable records, and historical `modelEvals` remain readable |
| WP45 | Bounded, resumable experiment runner | P0 | WP42, WP44; consume WP43 after it merges | new `convex/evalRunner*.ts` actions/jobs, eval-domain functions from WP44, `shared/evals.ts` + discovery checks, spend-limit integration, deterministic fixtures/tests | Generation, discovery, and pipeline experiments freeze versions/inputs/seed/price snapshots and run as idempotent resumable jobs with bounded concurrency, retries, cancellation, spend and tool-call caps; per-candidate failures preserve successful peers; counts reconcile; zero-key fixture runs complete; no production routing change |
| WP46 | Evaluation Lab shell + experiment setup UI | P0 | WP44 and stable WP45 start/status/cancel contracts | `src/app/(app)/evals/page.tsx`, `src/app/(app)/evals/new/page.tsx`, setup/table components under `src/components/app/evals/**`, eval server actions, app navigation | Operator-only `/evals` and `/evals/new` provide a dense Dark Chrome/Astryx table, filters, versioned dataset/catalog selection, budget/concurrency/case controls, cost preview, explicit create/start, progress/failure/cancel states; route and action boundaries authorize; accessible desktop/mobile browser checks pass |
| WP47 | Blind generation + discovery review UI | P0 | WP44, WP45, WP46 | `src/app/(app)/evals/[experimentId]/review/**`, review-only components under `src/components/app/evals/review/**`, judgment/assignment functions in eval domain | Stored-seed candidate order and labels remain blind until submission; generation supports A/B/tie/neither plus reason codes, discovery requires relevance with optional actionability/novelty/unsafe/stale/duplicate labels; independent reviews are auditable and cannot overwrite each other; keyboard and mobile/desktop checks pass |
| WP48 | Evaluation results, statistics + explicit decisions | P0 | WP45, WP47 | `src/app/(app)/evals/[experimentId]/page.tsx`, results components under `src/components/app/evals/results/**`, eval aggregation/export/decision functions, statistical helpers/tests | Aggregates reconcile to case outputs/judgments and show numerator, denominator, failures, exclusions, minimum-sample warnings and valid confidence intervals; drill-down and redacted authorized CSV/JSON export work; identities reveal only after review; promote-to-shadow/assisted, retest, or reject records evidence without mutating routing |
| WP49 | Shadow Grok discovery integration | P1 | WP43, WP44, WP45 | `convex/scannerActions.ts`, `convex/scanner.ts`, new shadow/provenance functions, additive `convex/schema.ts` fields/tables, `convex/spend.ts`, typed analytics/Sentry catalog, scanner tests | Deterministic capped sampling records cited/hydrated provenance, usage/cost/failures, and eval-run linkage under existing AI/X spend controls and a provider circuit breaker; default is `off` or `shadow`; shadow output never changes feed ordering, surfaced content, or notifications; fallback/availability are observable |
| WP50 | Owner-gated assisted discovery path | P1 | WP48, WP49, plus recorded owner approval of the experiment gate | `convex/scannerActions.ts`, `convex/opportunities.ts`, rollout/config authorization, feed opportunity row/detail components, focused ranking/filter/browser tests | A disabled-by-default allow-list can merge only hydrated Grok candidates through existing safety, relevance, dedupe, freshness, diversity, cooldown, saturation, deterministic score/reason, and opportunity paths; one switch returns to shadow/off; desktop/mobile flow passes; assisted is never globally enabled by the PR |
| WP51 | Production outcome bridge | P1/P2 | WP7, WP20, WP44–WP50; privacy/budget owner gates | additive eval/outcome fields in `convex/schema.ts`, eval/outcome linkage functions, `generatedReplies`/draft outcome paths, `shared/accountData.ts`, `convex/account.ts`, shadow automation and consent tests | Consented selected eval outputs link to observed selection, edit-distance bucket, sent, and responded outcomes without combining lab preference, judge, deterministic, and production metrics; export/deletion is complete; any shadow automation is budget/privacy bounded; no publish path or automatic routing mutation is introduced |
| WP52 | Compose activation and evidence | Beta / P2 validation | WP7, WP20, WP23, WP40 | `convex/compose.ts`, `convex/schema.ts`, `shared/compose.ts`, `src/app/actions.ts`, `src/lib/analytics/events.ts`, `src/components/app/compose-ladder.tsx`, sidebar/command navigation, compose/analytics tests, observability docs | Compose is hidden from navigation until a non-demo user has a real qualifying reply win while `/compose` remains directly reachable as an explicit zero-cost preview; real runs resolve authorized clusters server-side; input and generation provenance are truthful and persisted; typed events measure open → generate → option action and real/demo breakdowns; immediate-publish copy states that posting is queued rather than awaiting confirmation; demo mode, authorization, three-option guardrails, and full checks remain green |
| WP57 | Scanner background-cost guard | Cost / reliability P0 | None | `convex/schema.ts`, `convex/scanner.ts`, scanner-settings writers, scheduling helpers/tests, README, deployment verification | Background-cron eligibility is stored and indexed separately from the user-visible scanner toggle; demo users keep deterministic on-demand scanner behavior but never enter recurring scan fan-out; legacy rows have a dual-read rollout and idempotent backfill; the active dev deployment is backfilled/throttled and full checks are green |

| WP59 | CI dependency security + preview deploy pipeline | Pipeline P0 | None | `package.json`, `package-lock.json`, `vercel.ts`, `docs/production-deployment.md` | `npm audit --audit-level=high` is clean so the required CI `checks` job is green again; preview deployments no longer run a production Convex deploy and build successfully; `vercel.ts` selects its build command from `VERCEL_ENV` plus the `CONVEX_DEPLOY_KEY` prefix, so adding a `preview:` key later switches to per-branch Convex preview deployments with no code change; the deploy-key scoping that leaks a production credential into every preview build is documented as an owner action |

Program briefs: `docs/wp/WP24-ASTRYX-ADOPTION-PLAN.md` and
`docs/wp/WP40-FIRST-10-BETA-LAUNCH-GATE.md`, and
`docs/wp/WP41-GROK-EVAL-PROGRAM.md`. Rulings:
